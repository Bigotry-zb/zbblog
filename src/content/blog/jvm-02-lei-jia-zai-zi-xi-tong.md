...
![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024163558872.png)

#### 2、详细图

英文版

![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024164417433.png)

中文版

![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024164544639.png)
...
2.  ClassLoader只负责class文件的加载，至于它是否可以运行，则由Execution Engine（执行引擎）决定。  
    （比如：别人给你介绍对象，能不能成另说。其中ClassLoader只负责别人介绍对象，能不能成全看你）
3.  **加载的类信息存放于一块称为方法区的内存空间**。除了类的信息外，方法区中还会存放运行时常量池信息，可能还包括字符串字面量和数字常量（这部分常量信息是Class文件中常量池部分的内存映射）

![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024164911148.png)
...
![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024165223695.png)

### 3、类加载过程

![类加载过程](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024165836131.png)
...
![类加载过程示意图](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024170014984.png)
...
使用 BinaryViewer软件查看字节码文件，其开头均为 CAFE BABE （java），如果出现不合法的字节码文件，那么将会验证不通过。

![.class文件](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024171040995.png)
...
![查看符号引用](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024182616267.png)
...
7.  JDK7开始提供的动态语言支持：java.lang.invoke.MethodHandle实例的解析结果REF\_getStatic、REF putStatic、REF\_invokeStatic句柄对应的类没有初始化，则初始化

除了以上七种情况，其他使用Java类的方式都被看作是对类的被动使用，都不会导致类的初始化，即`不会执行初始化阶段（不会调用 clinit() 方法和 init() 方法）`
...
> IDEA 中安装 JClassLib Bytecode viewer 插件，可以很方便的看字节码。[IDEA安装JClass插件](https://blog.csdn.net/qq_42326620/article/details/122768642)

##### 1、有static变量

查看下面这个代码的字节码，可以发现有一个`<clinit>()`方法。

![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024185101090.png)
...
![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024185305810.png)

没有clinit是因为没有static变量

##### 3、构造器

![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024185513790.png)
...
![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024185655011.png)
...
![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024191612814.png)

Optional四者之间的关系是包含关系。不是上层下层，也不是子父类的 继承关系。

**ExtClassLoader**

![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024191738052.png)

**AppClassLoader**

![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221024191755771.png)
...
3.  在编写自定义类加载器时，如果没有太过于复杂的需求，可以`直接继承URIClassLoader类`，这样就可以避免自己去编写findclass()方法及其获取字节码流的方式，使自定义类加载器编写更加简洁。
...
![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221025162137614.png)

sun.misc.Launcher 它是一个java虚拟机的入口应用

![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221025162203691.png)

> 获取ClassLoader途径

![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221025162252275.png)
...
![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221025163844593.png)

> 举个通俗的例子：小孩有一个苹果，问妈妈说吃不吃，妈妈一看奶奶还在，就问奶奶吃不吃苹果；奶奶一看苹果有点硬，咬不动于是说不吃，给了妈妈，妈妈一看说苹果有点酸，又给了小孩，小孩说我正好喜欢吃又酸又硬的，于是小孩吃了。
...
1、自己建立一个 java.lang.String 类，写上 static 代码块
...
程序并没有输出我们静态代码块中的内容，说明仍然加载的是 JDK 自带的 String 类。
...
![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221025164730203.png)
...
![](/images/hexo/2022/10/24/jvm-02-lei-jia-zai-zi-xi-tong/image-20221025164859134.png)
...
简单来说，就是保证引导类加载器负责加载的核心包不会被恶意篡改
...
换句话说，在JVM中，即使这两个类对象（class对象）来源同一个Class文件，被同一个虚拟机所加载，但只要`加载它们的ClassLoader实例对象不同，那么这两个类对象也是不相等的`
